...
class Functions
{
	...
	public function get_price_detail($giacu = '', $giamoi = '', $giakm = '')
	{
?>
		<div class="price-detail">
			<span class="main-price-product"><?= ($giamoi) ? $this->format_money($giamoi) : $this->format_money($giacu) ?></span>
			<?= ($giamoi && $giacu) ? '<span class="main-price-compare">' . $this->format_money($giacu) . '</span>' : '' ?>
		</div>
		<?= ($giakm > 0 && $giakm < 100) ? '<div class="main-discount">(Tiết kiệm ' . $this->format_money($giacu - $giamoi) . ' ~ ' . $giakm . '%)</div>' : '' ?>
		<?= ($giamoi < 1 && $giacu < 1) ? '<a class="price-detail-contact" href="lien-he" title="' . lienhe . '">' . lienhe . '</a>' : '' ?>
	<?php
	}
...
?>
		<div class="<?= $col ?>">
			<div class="pro">
				<div class="img_pro img_">
					<a href="<?= $v[$sluglang] ?>" title="<?= $v['ten' . $lang] ?>">
						<img onerror="this.src='<?= THUMBS ?>/280x230x2/assets/images/noimage.png';" src="<?= THUMBS ?>/280x230x1/<?= UPLOAD_PRODUCT_L . $v['photo'] ?>" alt="<?= $v['ten' . $lang] ?>" />
					</a>
					<div class="ab_pro flex">
						<p class="flex">
							<span class="cart-add addcart " data-id="<?= $v['id'] ?>" data-action="addnow" data-tms="2"><i class="fa fa-shopping-cart" aria-hidden="true"></i></span>
							<a class="view_product" data-id="<?= $v['id'] ?>"><i class="fa fa-search" aria-hidden="true"></i></a>
							<?= $this->get_liked($v['id']); ?>
						</p>
					</div>
				</div>
				...
				<div class="price_pro">
					<?= $this->get_gia_pro($v['gia'], $v['giamoi']) ?>
				</div>
			</div>
		</div>
	<?php
	...
	public function get_liked($id = '')
	{
		global $lang, $login_member;
		$rt_lk = '<a class="like_product" data-login="1" data-id="' . $id . '" title="Like"><i class="fas fa-heart"></i></a>';
		if (isset($_SESSION[$login_member]['id'])) {
			$if_tv = $this->d->rawQueryOne("select liked,id from #_member where id = ? limit 0,1", array($_SESSION[$login_member]['id']));
			$rt_lk = '<a class="like_product" data-login="2" data-id="' . $id . '" title="Like"><i class="fas fa-heart"></i></a>';
		}
		if (isset($if_tv) && $if_tv['id'] != '') {

			$ds_liked[] = array();
			$ds_liked = explode(",", $if_tv['liked']);

			if (in_array($id, $ds_liked)) {
				$rt_lk = '<a class="like_product liked" data-login="2" data-id="' . $id . '" title="Liked"><i class="fas fa-heart"></i></a>';
			}
		}
	...
	?>
		<div class="<?= $col ?>">
			<div class="box_news flex_row">
				<div class="img_news img_">
					<a href="<?= $v[$sluglang] ?>" title="<?= $v['ten' . $lang] ?>">
						<img onerror="this.src='<?= THUMBS ?>/280x220x2/assets/images/noimage.png';" src="<?= THUMBS ?>/280x220x1/<?= UPLOAD_NEWS_L . $v['photo'] ?>" alt="<?= $v['ten' . $lang] ?>" />
					</a>
				</div>
				<div class="content_news">
					...
					<div class="date_news">Ngày đăng: <?= date("d/m/Y", $v['ngaytao']) ?></div>
					<div class="des_news">
...
<?php
	...
	public function orderStatus($status = 0)
	{
		...
		foreach ($row as $v) {
			...
			$str .= '<option value=' . $v["id"] . ' ' . $selected . '>' . $v["trangthaivi"] . '</option>';
		}
		$str .= '</select>';
