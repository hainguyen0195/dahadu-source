...
function get_item()
{
	global $d, $func, $strUrl, $curPage, $item, $gallery, $ds_size, $type, $com, $act;
	...
	$ds_size = $d->rawQuery("select * from #_size_color_price where id_product= ? order by stt desc ", array($id));
}
...
function save_item()
{
	...
	if ($id && $act != 'save_copy') {
		...
		if (isset($_POST['id_size_prd_have'])) {
			$size_daluu = array();
			$size_daluu = $_POST['id_size_prd_have'];
			for ($i = 0; $i < count($size_daluu); $i++) {
				$data2_up['id_size'] = $_POST['name-size_have'][$i];
				$data2_up['id_mau'] = $_POST['name-mau_have'][$i];
				$data2_up['gia'] = (isset($_POST['gia_size_prd_have'][$i]) && $_POST['gia_size_prd_have'][$i] != '') ? str_replace(",", "", $_POST['gia_size_prd_have'][$i]) : 0;
				$data2_up['giaban'] = (isset($_POST['giaban_size_prd_have'][$i]) && $_POST['giaban_size_prd_have'][$i] != '') ? str_replace(",", "", $_POST['giaban_size_prd_have'][$i]) : 0;
				$data2_up['ngaysua'] = time();
				$d->where('id', $size_daluu[$i]);
				$d->where('type', $type);
				$d->update('size_color_price', $data2_up);
			}
		}

		if (isset($_POST['name-size'])) {
			$array_size_add = array();
			$array_size_add = $_POST['name-size'];
			for ($i = 0; $i < count($array_size_add); $i++) {
				$data2_insert['id_size'] = $_POST['name-size'][$i];
				$data2_insert['id_mau'] = $_POST['name-mau'][$i];
				if ($data2_insert['id_size'] > 0 || $data2_insert['id_mau'] > 0) {
					$data2_insert['gia'] = (isset($_POST['gia_size_prd'][$i]) && $_POST['gia_size_prd'][$i] != '') ? str_replace(",", "", $_POST['gia_size_prd'][$i]) : 0;
					$data2_insert['giaban'] = (isset($_POST['giaban_size_prd'][$i]) && $_POST['giaban_size_prd'][$i] != '') ? str_replace(",", "", $_POST['giaban_size_prd'][$i]) : 0;
					$data2_insert['id_product'] = $id;
					$data2_insert['type'] = $type;
					$data2_insert['ngaytao'] = time();
					$data2_insert['hienthi'] = 1;
					$d->insert('size_color_price', $data2_insert);
				}
			}
		}
			...
			if (isset($_POST['name-size'])) {
				...
				for ($i = 0; $i < count($array_size_add); $i++) {
					...
					if ($data2_insert['id_size'] > 0 || $data2_insert['id_mau'] > 0) {
						$data2_insert['gia'] = (isset($_POST['gia_size_prd'][$i]) && $_POST['gia_size_prd'][$i] != '') ? str_replace(",", "", $_POST['gia_size_prd'][$i]) : 0;
						$data2_insert['giaban'] = (isset($_POST['giaban_size_prd'][$i]) && $_POST['giaban_size_prd'][$i] != '') ? str_replace(",", "", $_POST['giaban_size_prd'][$i]) : 0;
						$data2_insert['id_product'] = $id_insert;
						$data2_insert['type'] = $type;
						$data2_insert['ngaytao'] = time();
						$data2_insert['hienthi'] = 1;
						$d->insert('size_color_price', $data2_insert);
