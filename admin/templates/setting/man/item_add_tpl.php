							...
							<?php foreach($config['website']['lang'] as $k => $v) { ?>
								<div class="tab-pane fade show <?=($k=='vi')?'active':''?>" id="tabs-lang-<?=$k?>" role="tabpanel" aria-labelledby="tabs-lang">
									<div class="form-group">
										...
										<input type="text" class="form-control for-seo" name="data[ten<?=$k?>]" id="ten<?=$k?>" placeholder="Tiêu đề (<?=$k?>)" value="<?=(isset($item['ten'.$k]) && $item['ten'.$k] != '') ? $item['ten'.$k] : '' ?>" <?=($k=='vi')?'required':''?>>
									</div>
									<?php if(isset($config['setting']['slogan']) && $config['setting']['slogan'] == true) { ?>
										<div class="form-group">
											<label for="slogan">Slogan:</label>
											<input type="text" class="form-control" name="data[options][slogan<?=$k?>]" id="slogan<?=$k?>" placeholder="Slogan (<?=$k?>)" value="<?=(isset($options['slogan'.$k]) && $options['slogan'.$k] != '') ? $options['slogan'.$k] : '' ?>">
										</div>
									<?php } ?>
									<?php if(isset($config['setting']['diachi']) && $config['setting']['diachi'] == true) { ?>
										<div class="form-group">
											<label for="diachi">Địa chỉ:</label>
											<input type="text" class="form-control" name="data[options][diachi<?=$k?>]" id="diachi<?=$k?>" placeholder="Địa chỉ (<?=$k?>)" value="<?=(isset($options['diachi'.$k]) && $options['diachi'.$k] != '') ? $options['diachi'.$k] : '' ?>">
										</div>
									<?php } ?>
								</div>
							<?php } ?>
				...
				<div class="row">
					<?php if(isset($config['setting']['email']) && $config['setting']['email'] == true) { ?>
						<div class="form-group col-md-4 col-sm-6">
							...
							<input type="email" class="form-control" name="data[options][email]" id="email" placeholder="Email" value="<?=(isset($options['email']) && $options['email'] != '') ? $options['email'] : '' ?>">
						</div>
					<?php } ?>
					<?php if(isset($config['setting']['hotline']) && $config['setting']['hotline'] == true) { ?>
						<div class="form-group col-md-4 col-sm-6">
							<label for="hotline">Hotline:</label>
							<input type="text" class="form-control" name="data[options][hotline]" id="hotline" placeholder="Hotline" value="<?=(isset($options['hotline']) && $options['hotline'] != '') ? $options['hotline'] : '' ?>">
						</div>
					<?php } ?>
					<?php if(isset($config['setting']['dienthoai']) && $config['setting']['dienthoai'] == true) { ?>
						<div class="form-group col-md-4 col-sm-6">
							<label for="dienthoai">Điện thoại:</label>
							<input type="text" class="form-control" name="data[options][dienthoai]" id="dienthoai" placeholder="Điện thoại" value="<?=(isset($options['dienthoai']) && $options['dienthoai'] != '') ? $options['dienthoai'] : '' ?>">
						</div>
					<?php } ?>
					<?php if(isset($config['setting']['zalo']) && $config['setting']['zalo'] == true) { ?>
						<div class="form-group col-md-4 col-sm-6">
							<label for="zalo">Zalo:</label>
							<input type="text" class="form-control" name="data[options][zalo]" id="zalo" placeholder="Zalo" value="<?=(isset($options['zalo']) && $options['zalo'] != '') ? $options['zalo'] : '' ?>">
						</div>
					<?php } ?>
					<?php if(isset($config['setting']['oaidzalo']) && $config['setting']['oaidzalo'] == true) { ?>
						<div class="form-group col-md-4 col-sm-6">
							<label for="oaidzalo">OAID Zalo:</label>
							<input type="text" class="form-control" name="data[options][oaidzalo]" id="oaidzalo" placeholder="OAID Zalo" value="<?=(isset($options['oaidzalo']) && $options['oaidzalo'] != '') ? $options['oaidzalo'] : '' ?>">
						</div>
					<?php } ?>
					<?php if(isset($config['setting']['website']) && $config['setting']['website'] == true) { ?>
						<div class="form-group col-md-4 col-sm-6">
							<label for="website">Website:</label>
							<input type="text" class="form-control" name="data[options][website]" id="website" placeholder="Website" value="<?=(isset($options['website']) && $options['website'] != '') ? $options['website'] : '' ?>">
						</div>
					<?php } ?>
					<?php if(isset($config['setting']['fanpage']) && $config['setting']['fanpage'] == true) { ?>
						<div class="form-group col-md-4 col-sm-6">
							<label for="fanpage">Fanpage:</label>
							<input type="text" class="form-control" name="data[options][fanpage]" id="fanpage" placeholder="Fanpage" value="<?=(isset($options['fanpage']) && $options['fanpage'] != '') ? $options['fanpage'] : '' ?>">
						</div>
					<?php } ?>
					<?php if(isset($config['setting']['messenger']) && $config['setting']['messenger'] == true) { ?>
						<div class="form-group col-md-4 col-sm-6">
							<label for="messenger">messenger:</label>
							<input type="text" class="form-control" name="data[options][messenger]" id="messenger" placeholder="messenger" value="<?=(isset($options['messenger']) && $options['messenger'] != '') ? $options['messenger'] : '' ?>">
						</div>
					<?php } ?>
					<?php if(isset($config['setting']['toado']) && $config['setting']['toado'] == true) { ?>
						<div class="form-group col-md-4 col-sm-6">
							<label for="toado">Tọa độ google map:</label>
							<input type="text" class="form-control" name="data[options][toado]" id="toado" placeholder="Tọa độ google map" value="<?=(isset($options['toado']) && $options['toado'] != '') ? $options['toado'] : '' ?>">
						</div>
					<?php } ?>
					<?php if(isset($config['setting']['time_open']) && $config['setting']['time_open'] == true) { ?>
						<div class="form-group col-md-4 col-sm-6">
							<label for="time_open">Thời gian mở cửa:</label>
							<input type="text" class="form-control" name="data[options][time_open]" id="time_open" placeholder="Thời gian mở cửa" value="<?=(isset($options['time_open']) && $options['time_open'] != '') ? $options['time_open'] : '' ?>">
						</div>
					<?php } ?>
					<?php if(isset($config['setting']['coppyright']) && $config['setting']['coppyright'] == true) { ?>
						<div class="form-group col-md-4 col-sm-6">
							<label for="coppyright">Coppyright:</label>
							<input type="text" class="form-control" name="data[options][coppyright]" id="coppyright" placeholder="Coppyright" value="<?=(isset($options['coppyright']) && $options['coppyright'] != '') ? $options['coppyright'] : '' ?>">
						</div>
					<?php } ?>
					<?php if(isset($config['setting']['paging_product']) && $config['setting']['paging_product'] == true) { ?>
						<div class="form-group col-md-4 col-sm-6">
							...
							<input type="text" class="form-control" name="data[options][paging_product]" id="paging_product" placeholder="Phân trang sản phẩm" value="<?=(isset($options['paging_product']) && $options['paging_product'] != '') ? $options['paging_product'] : '' ?>">
						</div>
					<?php } ?>
					<?php if(isset($config['setting']['paging_news']) && $config['setting']['paging_news'] == true) { ?>
						<div class="form-group col-md-4 col-sm-6">
							<label for="paging_news">Phân trang tin tức:</label>
							<input type="text" class="form-control" name="data[options][paging_news]" id="paging_news" placeholder="Phân trang tin tức" value="<?=(isset($options['paging_news']) && $options['paging_news'] != '') ? $options['paging_news'] : '' ?>">
						</div>
					<?php } ?>
					<?php if(isset($config['setting']['paging_index']) && $config['setting']['paging_index'] == true) { ?>
						<div class="form-group col-md-4 col-sm-6">
							...
							<input type="text" class="form-control" name="data[options][paging_index]" id="paging_index" placeholder="Phân trang trang chủ" value="<?=(isset($options['paging_index']) && $options['paging_index'] != '') ? $options['paging_index'] : '' ?>">
						</div>
					<?php } ?>
				</div>
