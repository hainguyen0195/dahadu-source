...
<script type="text/javascript">
	jQuery(document).ready(function($) {
		//var name_size = $("#name-size").clone();

		$('.clickaddsize').click(function() {

			$('#addsize').append('<div class="formRight item_mau_show"><div class="size_1"><select name="name-size[]" class="form-control select2" id="name-size"><option>Chọn size</option><?php foreach ($ds_size_s as $k => $v) { ?><option value="<?= $v['id'] ?>" ><?= $v['tenvi'] ?></option><?php } ?></select></div> <div class="size_1"><select name="name-mau[]" class="form-control select2" id="name-mau"><option>Chọn màu</option><?php foreach ($ds_mau_s as $k => $v) { ?><option value="<?= $v['id'] ?>" ><?= $v['tenvi'] ?></option><?php } ?></select></div><div class="size_2"><input type="text" name="gia_size_prd[]" class="textsize_prd_new conso form-control" autocomplete="off" placeholder="Nhập giá bán" /></div> <div class="size_3"><input type="text" name="giaban_size_prd[]" class="textsize_prd_new conso form-control" autocomplete="off" placeholder="Nhập giá mới"/></div><div class="size_4"><a class="delete_images_size_ap text-danger"><i class="fas fa-trash-alt"></i></a></div> </div> ');
			load_conso();
		});
	});
</script>
...
<div class="card card-primary card-outline text-sm">
	<div class="card-header">
		<h3 class="card-title">Thuộc tính size <?= $config['product'][$type]['title_main'] ?></h3>
					...
					<div class="formRight">
						<?php for ($i = 0; $i < count($ds_size); $i++) { ?>
							<div class="item_mau_show">
								...
								<div class="size_1">
									<select name="name-size_have[]" class="form-control select2" id="name-size_have">
								...
								<div class="size_1">
									<select name="name-mau_have[]" class="form-control select2" id="name-mau_have">
